...
<script>
    ...
    var toolbar = [{
        ...
        iconCls:'icon-search',
        handler:function(){
            if(document.getElementById("searchById").value.length>0&&document.getElementById("searchById").value!=""){
                $.post("/item/"+document.getElementById("searchById").value, {searchById:document.getElementById("searchById").value},
                function (data) {
                    var obj = {"total":2,"rows":[data]};
                    $('#itemList').datagrid('loadData',obj);
                },
                "json")
            }
        }
    },{
        text:"<input type=\"text\" name=\"searchById\" id=\"searchById\" placeholder=\"请输入商品ID\">"
    }];
</script>
