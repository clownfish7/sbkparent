...
<div class="linknav">
    <div class="schArticle">
        <a href="/article/search?keyword=%E6%9C%88%E9%A5%BC" target="_blank">找到和“<span>${query}</span>”相关的商品信息共<span
                id="articlenum">${totalPages }</span>页&gt;&gt;
        </a>
    </div>
    ...
    <div class="main-box">
        ...
        <div class="p-list">
            <%--<c:forEach items="${itemList }" var="item">
                <h6>${item.id}--${item.image}--${item.sell_point}--${item.title}--${item.price}--</h6>
            </c:forEach>--%>
            <ul class="list-all">
                <c:forEach items="${itemList }" var="item">
                    <li>
                        <div class="l-wrap">
                            <div class="pic">
                                <a class="trackref" href="http://localhost:8087/item/${item.id}.html" title=""
                                   target="_blank">
                            ...
                            <div class="price">
                                     <span>
                                         <span class="p-now">￥<strong><fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${item.price / 100 }"/></strong></span>
                                         <span class="p-nor"></span>
                                         <span class="active" style="">直降</span>
                                     </span>
                            </div>
                            <div class="title-a">
                                <a class="trackref presaleSign_225865" href="http://localhost:8087/item/${item.id}.html"
                                   target="_blank">${item.title }</a>
                            </div>
                            <div class="title-b" style="">
                                <a class="trackref"
                                   href="http://localhost:8087/item/${item.id}.html"
                                   target="_blank">${item.sell_point}
                                </a>
                            </div>
                            <div class="comment">
